...
class AnnotatedTypeFormatter {
    ...
    supportsType(type) {
        return type instanceof AnnotatedType_js_1.AnnotatedType;
    }
    getDefinition(type) {
        const annotations = type.getAnnotations();
        if ("discriminator" in annotations) {
            const deref = (0, derefType_js_1.derefType)(type.getType());
            if (deref instanceof UnionType_js_1.UnionType) {
                deref.setDiscriminator(annotations.discriminator);
                delete annotations.discriminator;
            }
            else {
                throw new Errors_js_1.JsonTypeError(`Cannot assign discriminator tag to type: ${deref.getName()}. This tag can only be assigned to union types.`, deref);
            }
        }
        const def = {
