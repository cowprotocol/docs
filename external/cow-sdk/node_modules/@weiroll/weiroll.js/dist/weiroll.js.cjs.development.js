...
var StateValue = function StateValue() {
  this.param = abi.ParamType.from('bytes[]');
};
...
var FunctionCall = /*#__PURE__*/function () {
  ...
  _proto.withValue = function withValue(value) {
    ...
    return new FunctionCall(this.contract, this.flags & ~CommandFlags.CALLTYPE_MASK | CommandFlags.CALL_WITH_VALUE, this.fragment, this.args, encodeArg(value, abi.ParamType.from('uint')));
  };
...
function abiEncodeSingle(param, value) {
  if (isDynamicType(param)) {
    return new LiteralValue(param, bytes.hexDataSlice(abi.defaultAbiCoder.encode([param], [value]), 32));
  }

  return new LiteralValue(param, abi.defaultAbiCoder.encode([param], [value]));
}
...
var BaseContract = /*#__PURE__*/function () {
  function BaseContract(address, contractInterface, commandflags) {
    ...
    this["interface"] = properties.getStatic(this instanceof BaseContract ? this.constructor : void 0, 'getInterface')(contractInterface);
    ...
    Object.keys(this["interface"].functions).forEach(function (signature) {
      ...
      if (_this[signature] == null) {
        properties.defineReadOnly(_this, signature, buildCall(_this, fragment));
      ...
      if (_this.functions[signature] == null) {
        properties.defineReadOnly(_this.functions, signature, buildCall(_this, fragment));
      }
    });
    Object.keys(uniqueNames).forEach(function (name) {
      ...
      try {
        if (_this[name] == null) {
          properties.defineReadOnly(_this, name, _this[signature]);
        }
      } catch (e) {}

      if (_this.functions[name] == null) {
        properties.defineReadOnly(_this.functions, name, _this.functions[signature]);
      }
    });
  }
...
var Planner = /*#__PURE__*/function () {
  ...
  _proto2.add = function add(call) {
    ...
    if (call.flags & CommandFlags.TUPLE_RETURN) {
      return new ReturnValue(abi.ParamType.fromString('bytes'), command);
    }
  ...
  _proto2.buildCommands = function buildCommands(ps) {
    ...
    for (var _iterator5 = _createForOfIteratorHelperLoose(this.commands), _step5; !(_step5 = _iterator5()).done;) {
      ...
      if (command.type === CommandType.SUBPLAN) {
        ...
        ps.state.push(bytes.hexDataSlice(abi.defaultAbiCoder.encode(['bytes32[]'], [subcommands]), 32)); // The slot is no longer needed after this command
      ...
      if ((flags & CommandFlags.EXTENDED_COMMAND) === CommandFlags.EXTENDED_COMMAND) {
        // Extended command
        encodedCommands.push(bytes.hexConcat([command.call.contract["interface"].getSighash(command.call.fragment), [flags, 0, 0, 0, 0, 0, 0, ret], command.call.contract.address]));
        encodedCommands.push(bytes.hexConcat([padArray(args, 32, 0xff)]));
      } else {
        // Standard command
        encodedCommands.push(bytes.hexConcat([command.call.contract["interface"].getSighash(command.call.fragment), [flags], padArray(args, 6, 0xff), [ret], command.call.contract.address]));
      }
    }
...
//# sourceMappingURL=weiroll.js.cjs.development.js.map
