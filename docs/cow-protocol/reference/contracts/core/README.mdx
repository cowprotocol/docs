...
### Disabled settlement path

`GPv2Settlement.swap()` belongs to an older Balancer settlement flow that is no longer supported on Ethereum. Use the standard `settle()` flow instead.
