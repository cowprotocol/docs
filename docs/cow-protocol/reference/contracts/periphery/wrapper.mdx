...
```
┌─────────────────────┬──────┬──────────┬──────────┬──────┬──────────┐
│   Settle Calldata   │ Len₁ │  Data₁   │  Addr₂   │ Len₂ │  Data₂   │
│   (to settlement)   │(2 B) │ (wrap1)  │ (20 B)   │(2 B) │ (wrap2)  │
└─────────────────────┴──────┴──────────┴──────────┴──────┴──────────┘
...
Note: The settlement contract address is derived from the `SETTLEMENT` immutable and is not included in `wrapperData`.
...
See the [CowWrapperHelper contract source code](https://github.com/cowprotocol/euler-integration-contracts/blob/main/src/CowWrapperHelper.sol) for the complete implementation.
...
`validateWrapperData()` must always return same result for same input:
...
When settlement contract upgrades, all wrappers must be redeployed. It is recommended to use [Cannon](https://usecannon.com/learn) to make it easy to redeploy your wrappers.
