...
* The Safe borrows 10,000 USDT on Aave using 400,000+ USDC as collateral.
...
4. Perform a trade on CoW Swap, selling up to 400,000 USDC for 5,002.5 USDT (borrowed amount + [0.05% flash loan fee on Aaave](https://aave.com/docs/concepts/flash-loans))
2. Use the 5,002.5 USDT from the trade to fully repay the flash loan (including the fee).

:::info

Liquidity and exchange ratios on the Sepolia network can be very inconsistent. To ensure successful execution of the order, we use a high amount of USDC (both as collateral and as the token being traded).

General considerations when testing flash loans on Sepolia and Aave:
- [Token names in Aaave may not correspond](https://github.com/mrnaveira/cow-flashloan-tester?tab=readme-ov-file#2-select-aave-tokens) with tokens names in other dapps.
...
- Try a different RPC in case of transaction submission issues.
...
- On Aave: 
    - The USDT debt is reduced from 10k to 5k.
    - The USDC collateral is reduced by 400k.
