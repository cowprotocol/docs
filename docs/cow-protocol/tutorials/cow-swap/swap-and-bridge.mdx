...
**Tokens must be different.** Bridging the same token between two networks is not yet supported. This feature is under development.
...
* [EIP-6963](https://eips.ethereum.org/EIPS/eip-6963) permit flow (when supported)
...
* **Expected deposit amount** → This is equal to the “Expected to receive” amount from the swap stage.
* **Bridge cost** → A fee charged by the bridge provider for processing your transfer.
* **Estimated bridging time** → Pulled directly from the bridge provider (varies by network and provider).
* **Minimum to receive**:

  - Calculated as: `(Swap min. to receive)` - `(Bridge provider cost)`
  - `Swap min. to receive` = final amount after all swap fees, gas costs, and slippage tolerance.
...
The **Account Proxy** (also known as **CoW Shed**) is a helper contract that powers advanced features like [**CoW Hooks**](/cow-protocol/reference/core/intents/hooks).
...
* The received amount is sent to your **Account Proxy**.
...
Most bridge providers will automatically send your funds back to your **Account Proxy** once the deposit expires.
...
* Go to the CoW Swap [**Account Proxy**](https://swap.cow.fi/#/1/account-proxy/help) page and withdraw to your wallet.
* There is a list of Account Proxy versions, in the most cases you need to use **latest** one unless told otherwise.
...
* The explorer shows orders in all states:
**In progress**, **Completed**, **Cancelled**, and more.
* You can filter and check full details of each trade.
