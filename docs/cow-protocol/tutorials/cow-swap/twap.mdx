...
- **Reduce price impact on large orders**: e.g., Buy `$1M` of ETH over 3h in 6 parts; The TWAP will place an order of `$166k` every `30min` significantly reducing the price impact and allowing the market to recover.
