...
| ------------------------------------- | ---------------------- | ------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `appCode`                             | `string`               | ---                 | The unique identifier of the widget consumer. Please fill the form to let us know a little about you: <https://cowprotocol.typeform.com/to/rONXaxHV>.                                                                                                                                                                                                                                        |
...
| `maxHeight`                           | `number`               | ---                 | The widget automatically adjusts its height depending on the content. By default, the maximum height is `body.offsetHeight`, but it can be customized using the `maxHeight` parameter.                                                                                                                                                                                                       |
...
| `targetChainId`                       | `number`               | ---                 | The ID of a network to bridge the output token to.                                                                                                                                                                                                                                                                                                                                           |
| `tradeType`                           | `TradeType`            | `'swap'`            | The type of trade. Can be `swap`, `limit`, or `advanced`.                                                                                                                                                                                                                                                                                                                                    |
| `sell`                                | `TradeAsset`           | Same as swap.cow.fi | An object containing information about the sell asset. Example: `{ asset: 'WBTC', amount: '12' }` or `{ asset: '0xa0b86991c6218b36c1d19d4a2e9eb0ce3606eb48' }`.                                                                                                                                                                                                                              |
...
| `forcedOrderDeadline`                 | `ForcedOrderDeadline`  | ---                 | Order deadline in minutes. Either a single value applied to each individual order type accordingly or an optional individual value per order type. When set, users won't be able to edit the deadline. Accepted values: single for all `123` OR per TradeType `{"swap": 15, "limit": 666}` OR per network `{1: 35, 100: 5}`.                                                                  |
| `baseUrl`                             | `string`               | https://swap.cow.fi | The base URL of the widget. Defaults to the production environment. Only `https` origins are accepted (except `http` on local dev hosts such as `localhost` or `127.0.0.1`).                                                                                                                                                                                                                 |
| `trustedOrigin`                       | `string`               | ---                 | Overrides the origin trusted for postMessage communication with the widget iframe. Defaults to the origin derived from `baseUrl`.                                                                                                                                                                                                                                                             |
| `locale`                              | `string`               | ---                 | Forces the widget locale. Accepts supported locales like `en-US` or fuzzy values like `en`.                                                                                                                                                                                                                                                                                                  |
| `theme`                               | `CowSwapTheme`         | `'light'`           | The theme of the widget (`'dark'` or `'light'`). You can also pass a `CowSwapWidgetPalette` object for custom colors. See [Custom theme](#custom-theme) section for more details.                                                                                                                                                                                                            |
| `tokenLists`                          | `string[]`             | ---                 | CoW Swap uses the [token lists standard](https://tokenlists.org) for flexible token management. Using this option you can set any token list you want. See also `customTokens` for programmatic token addition.                                                                                                                                                                               |
| `sellTokenLists`                      | `string[]`             | ---                 | Token lists (as URLs) to use in the sell token selector. Also contributes to the globally enabled list set. Falls back to `tokenLists` if omitted.                                                                                                                                                                                                                                           |
| `buyTokenLists`                       | `string[]`             | ---                 | Token lists (as URLs) to use in the buy token selector. Also contributes to the globally enabled list set. Falls back to `tokenLists` if omitted.                                                                                                                                                                                                                                            |
| `customTokens`                        | `TokenInfo[]`          | ---                 | Use tokens not in the default token lists by passing an array of ERC-20 token info objects. See also `tokenLists` for list-based configuration.                                                                                                                                                                                                                                              |
...
| `slippage`                            | `SlippageConfig`       | ---                 | Sets slippage settings for the widget. See [Slippage configuration](#slippage-configuration) section for more details.                                                                                                                                                                                                                                                                       |
...
| `tokenPairConstraints`                | `object[]`             | ---                 | Restricts trading to specific token pairs. Each entry is `{ sell: { address, chainId }, buy: { address, chainId } }`. See [Token pair constraints](#token-pair-constraints) section for more details.                                                                                                                                                                                        |
| `disableToastMessages`                | `boolean`              | false               | Disables pop-up notifications in the widget. You may want to handle these events yourself via the events API. See [Events handling](#events-handling) section for more details.                                                                                                                                                                                                              |
| `disablePostedOrderConfirmationModal` | `boolean`              | false               | Disables the confirmation modal shown after posting an order.                                                                                                                                                                                                                                                                                                                                |
...
| `hideOrdersTable`                     | `boolean`              | false               | Hides the orders table on limit and advanced forms. Warning! When `true`, users won't be able to see their order status, history, or cancel active orders.                                                                                                                                                                                                                                   |
...
## Token pair constraints

The `tokenPairConstraints` parameter lets you restrict the widget so that only specific token pairs can be traded. When set, the sell and buy token selectors will be locked to the configured pairs.

Each entry in the array specifies a sell token and a buy token by their contract address and chain ID:

```typescript
import { createCowSwapWidget, CowSwapWidgetParams, SupportedChainId } from '@cowprotocol/widget-lib'

const params: CowSwapWidgetParams = {
  appCode: 'YOUR_APP_ID',
  tokenPairConstraints: [
    {
      sell: { address: '0xa0b86991c6218b36c1d19d4a2e9eb0ce3606eb48', chainId: SupportedChainId.MAINNET }, // USDC
      buy: { address: '0xc02aaa39b223fe8d0a0e5c4f27ead9083c756cc2', chainId: SupportedChainId.MAINNET },  // WETH
    },
    {
      sell: { address: '0x6b175474e89094c44da98b954eedeac495271d0f', chainId: SupportedChainId.MAINNET }, // DAI
      buy: { address: '0xc02aaa39b223fe8d0a0e5c4f27ead9083c756cc2', chainId: SupportedChainId.MAINNET },  // WETH
    },
  ],
}
...
When `tokenPairConstraints` contains only one entry, the widget locks directly to that pair. When multiple entries are provided, the user can switch between them but cannot select arbitrary tokens.
