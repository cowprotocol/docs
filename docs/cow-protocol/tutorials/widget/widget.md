...
| `sounds`               | `CowSwapWidgetSounds`  | ---                    | The famous MOOO! sound can be changed to something else, just provide a link to the media file (mp3). You can also specify a value of null to disable the sound.                                                                                                                                                            |
...
| `tokenLists`           | `string[]`             | ---                    | CowSwap uses the [token lists standard](https://tokenlists.org), which allows flexible and decentralized management of assets for trading.                                                                                                                                                                                  |
| `customTokens`         | `TokenInfo[]`          | ---                    | In the case when you want to use tokens that are not included in the default token lists, you can add them programmatically by simply passing an array of ERC-20 tokens info.                                                                                                                                               |
...
const params: CowSwapWidgetParams = {
    tokenLists: ['https://gateway.ipfs.io/ipns/tokens.uniswap.org']
}
