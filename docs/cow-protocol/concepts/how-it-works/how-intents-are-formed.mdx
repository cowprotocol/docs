...
An [_intent_](https://docs.cow.finance/cow-protocol/concepts/introduction/intents) in CoW Protocol is a signed message that represents a user's wish to trade. It doesn't execute a trade directly — instead, it delegates execution to [solvers](../introduction/solvers) who find the optimal path.
...
```
POST https://api.cow.finance/mainnet/api/v1/quote
```
